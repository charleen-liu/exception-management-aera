...
<div class="bn-sidenav">
  ...
  const currentStep = pageFileMap[currentFile] !== undefined ? pageFileMap[currentFile] : -1;
  ...
  if (currentStep >= 0) {
    ...
    demoSteps.forEach(function(step, i) {
      // dot
      ...
      dot.style.cssText = [
        ...
        'background:' + (isActive ? '#6b3fa0' : isPast ? '#c4a8e8' : '#d9d9d9'),
        'flex-shrink:0',
      ].join(';');
      dot.title = step.label;
      if (!isActive) {
        dot.onclick = function() { window.location = step.href; };
        dot.onmouseenter = function() { dot.style.background = '#9b6fd0'; };
        dot.onmouseleave = function() { dot.style.background = isPast ? '#c4a8e8' : '#d9d9d9'; };
      }
    ...
    if (currentStep > 0) {
      ...
      prev.textContent = '‹';
      prev.style.cssText = 'background:none;border:none;cursor:pointer;font-size:16px;color:#6b3fa0;padding:0 4px;line-height:1;font-family:inherit;';
      prev.onclick = function() { window.location = demoSteps[currentStep - 1].href; };
      stepInner.insertBefore(prev, stepInner.firstChild);
    }

    if (currentStep < demoSteps.length - 1) {
      var next = document.createElement('button');
      next.textContent = '›';
      next.style.cssText = 'background:none;border:none;cursor:pointer;font-size:16px;color:#6b3fa0;padding:0 4px;line-height:1;font-family:inherit;';
      next.onclick = function() { window.location = demoSteps[currentStep + 1].href; };
      stepInner.appendChild(next);
    }

    stepBar.appendChild(stepInner);
